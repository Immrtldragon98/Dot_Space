...
devicesRouter.delete('/:sessionId',async(req:AuthRequest,res)=>{const sessionId=routeParam(req.params.sessionId);const ok=await revokeSession(req.userId!,sessionId);if(ok)revokeSessionRealtime(sessionId);if(!ok)return res.status(404).json({error:'Active session not found'});res.status(204).end();});
